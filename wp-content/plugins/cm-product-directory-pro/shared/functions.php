...
if (!function_exists('cmpd_parse_php_info')) {

    function cmpd_parse_php_info() {
...
if (!function_exists('cmpd_file_exists_remote')) {
     ...
     * @return boolean
     */
    function cmpd_file_exists_remote($url) {
...
if (!function_exists('cmpd_sort_WP_posts_by_title_length')) {

    function cmpd_sort_WP_posts_by_title_length($a, $b) {
        $sortVal = 0;
...
if (!function_exists('cmpd_strip_only')) {
     ...
     * @return type
     */
    function cmpd_strip_only($str, $tags, $stripContent = false) {
        $content = '';
        if (!is_array($tags)) {
...
if (!function_exists('cmpd_truncate')) {
     ...
     * @return string
     */
    function cmpd_truncate($text, $length = 100, $ending = '...', $exact = false, $considerHtml = true) {
        if (is_array($ending)) {
            extract($ending);
        }
        if ($considerHtml) {
...
if (!function_exists('cmpd_show_message')) {
     ...
     */
    function cmpd_show_message($message, $errormsg = false) {
        if ($errormsg) {
...
if (!function_exists('cmpd_units2bytes')) {
...
if (!function_exists('cmpd_paginate_links')) {

    function cmpd_paginate_links($args = '') {
        $defaults = array(
        ...
        if ($prev_next && $current && ( $current < $total || -1 == $total )) :
            ...
            /** This filter is documented in wp-includes/general-template.php */
            $page_links[] = '<a class="next page-numbers cmpd-button' . $link_class . '" href="' . esc_url(apply_filters('paginate_links', $link)) . '">' . $next_text . '</a>';
        endif;
        switch ($type) :
            case 'array' :
                return $page_links;
                break;
            case 'list' :
                $r .= '<ul class="page-numbers cmpd-button">\n\t<li>';
                $r .= join("</li>\n\t<li>", $page_links);
                $r .= "</li>\n</ul>\n";
                break;
            default :
