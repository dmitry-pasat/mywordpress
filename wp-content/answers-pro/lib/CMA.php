...
class CMA {
    ...
    public static function init($pluginFilePath)
    {
        ...
        add_action('plugins_loaded', array('CMA_MicroPaymentsBadges', 'plugins_loaded'));
        add_action('plugins_loaded', array('CMA_RejectedContentNotification', 'plugins_loaded'));
        add_action('plugins_loaded', array('CMA_Labels', 'bootstrap'));
    ...
    public static function getPostingUserId() {
    	if (is_user_logged_in()) {
    		...
    		return self::getAnonymousPostingUserId();
    	}
    }
    
    
    public static function getAnonymousPostingUserId() {
    	return apply_filters('cma_anonymous_user_id', 0);
    }
...
//     	$refererPostId = url_to_postid($_SERVER['HTTP_REFERER']);
