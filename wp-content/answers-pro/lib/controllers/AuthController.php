...
//             var_dump($response);
    ...
    protected static function _authenticate($auth = array())
    {
        ...
        $name       = $auth['info']['name'];
        $wp_user_id = $wpdb->get_var($wpdb->prepare("SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key = %s AND meta_value = %s",
        	'_cma_uid_'. strtolower($provider), $uid));
