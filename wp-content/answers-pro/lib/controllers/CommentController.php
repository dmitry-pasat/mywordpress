...
// 				break;
	...
	protected static function _processCommentAdd() {
		...
		$content = stripslashes(self::_getParam('content'));
		...
		if (empty($_POST['nonce']) OR !wp_verify_nonce($_POST['nonce'], 'cma_comment')) {
		...
		else if (!$thread->canPostComments()) {
			$error = CMA_Labels::getLocalized('error_cannot_post_comments');
		}
			...
			else if (!$answer->canPostComments()) {
				$error = CMA_Labels::getLocalized('error_cannot_post_comments');
			}
		}
	...
	protected static function displayComments($threadId, $answerId) {
		...
		if ($answerId) {
			if ($answer = CMA_Answer::getById($answerId)) {
				$canPostComments = $answer->canPostComments();
			}
		} else {
			if ($thread = CMA_Thread::getInstance($threadId)) {
				$canPostComments = $thread->canPostComments();
			}
		}
		
		echo self::_loadView('answer/comments/comments-block', compact('comments', 'threadId', 'answerId', 'canPostComments'));
	}
