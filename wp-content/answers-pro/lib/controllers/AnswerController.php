...
//     	add_action('pre_get_posts', array(get_class(), 'registerAdminCustomOrder'), 98, 1);
    	...
    	add_filter('template_include', array(__CLASS__, 'fixBuddyPressGlobalSearch'), 20, 1);
    	add_action('cma_questions_index_bottom', array(__CLASS__, 'cma_questions_index_bottom'), 10, 1);
    	
    	CMA_ThemeCustomizer::initialize(array(
		    	'general' => array('referrals'),
    	...
    	wp_register_style('CMA-css', CMA_URL . '/views/resources/app.css', array('dashicons'), CMA::version());
    	
        add_filter('posts_search', array(get_class(), 'alterSearchQuery'), 9999, 2);
        add_filter('posts_search', array(get_class(), 'supportTagsSearch'), 99999, 2);
        ...
        add_action('cma_edit_question_attachments', array(get_class(), 'cma_edit_question_attachments'), 10, 1);
        add_action('cma_edit_answer_attachments', array(get_class(), 'cma_edit_answer_attachments'), 10, 1);
        ...
        add_action('cma_index_questions_top', array(__CLASS__, 'cma_index_questions_top'), 10, 1);
    ...
    public static function getTemplatePart($slug, $name = null) {
    	...
    	switch ($name) {
    		...
    		case 'attachments':
    			...
    			echo apply_filters('cma_thread_attachments_list', self::_loadView('answer/meta/attachments', compact('thread', 'attachments')), $thread);
    			break;
    		...
    		case 'rating-answer':
    			...
    			echo self::_loadView('answer/meta/rating-answer',
    				compact('thread', 'rating', 'bestAnswerLabel', 'answer'));
    			break;
    		case 'content-answer':
    			global $answer;
    			$content = do_shortcode(CMA_Thread::filterShortcodes(CMA_VideoHelper::processContent($answer->getContent())));
    			echo apply_filters('cma_content_answer', self::_loadView('answer/content-answer', compact('thread', 'answer', 'content')), $thread, $answer);
    			break;
    		case 'attachments-answer':
    			global $answer;
    			$attachments = $answer->getAttachments();
    			echo apply_filters('cma_answer_attachments_list', self::_loadView('answer/meta/attachments', compact('thread', 'answer', 'attachments')), $answer);
    			break;
    		case 'meta-answer':
    			global $answer;
    			if ($user = $answer->getAuthor()) {
    				$avatar = $user->getAvatarHtml();
    			} else {
    				$avatar = null;
    			}
...
//     			}
    ...
    public static function getBacklink() {
		...
		return sprintf('<div class="cma-backlink-wrapper"><a class="cma-backlink" href="%s"> &laquo; %s</a></div>', esc_attr($backlinkUrl), CMA_Labels::getLocalized('back_to_previous_page'));
    }
    ...
    public static function registerCustomFilter(WP_Query $query) {
    	if( !is_admin() AND isset($query->query_vars['post_type']) && $query->query_vars['post_type'] == CMA_Thread::POST_TYPE ) {
    		...
    		if (CMA_Settings::getOption(CMA_Settings::OPTION_RESTRICT_UNANSWERED_QUESTIONS_TO_EXPERTS)) {
    			add_filter('posts_where_request', array(__CLASS__, 'unansweredQuestionToExpertsFilter'));
    		}
    ...
    static function unansweredQuestionToExpertsFilter($sql) {
    	global $wpdb;
//     	return $sql;
    	$userId = intval(get_current_user_id());
    	$expertsCategoriesTTIds = ($userId ? CMA_Category::getExpertsTermTaxonomyIds($userId) : array());
    	$sql .= ' AND (
			comment_count > 0 OR post_author = ' . intval($userId) . ' ';
	    	if (count($expertsCategoriesTTIds) > 0) {
				$sql .= ' OR EXISTS ('. CMA_Thread::getCategoriesSubquery($wpdb->posts, CMA_Category::TAXONOMY, 'term_taxonomy_id') .'
						AND cma_twc_tax.term_taxonomy_id IN ('. implode(',', $expertsCategoriesTTIds) .'))';
	    	}
	    	$sql .= ' OR NOT EXISTS ('. CMA_Thread::getCategoriesSubquery($wpdb->posts, CMA_Category::TAXONOMY, 'term_taxonomy_id') .')';
		$sql .= ')';
    	return $sql;
    }
    ...
    public static function thePostsFilter($posts) {
    	...
    	remove_filter('posts_where_request', array(__CLASS__, 'unansweredQuestionToExpertsFilter'));
    	return $posts;
    }
...
// echo '<br><br><br><br><br><br><br><br>';
    	...
    	$val .= ' AND (ID IN ('. CMA_Thread::getCategoryAccessFilterSubquery() .')
						...
						OR NOT EXISTS ('. CMA_Thread::getCategoriesSubquery($wpdb->posts, CMA_Category::TAXONOMY).')
...
//     	$redirectAfterPost = '_thread';
    	$cmaPageContent = CMA_BaseController::_loadView('answer/widget/question-form',
    			compact('post', 'thread', 'edit', 'populatedData'));
    ...
    protected static function _processEditAnswerView() {
    	...
    	$edit = true;
    	$cmaPageContent = CMA_BaseController::_loadView('answer/widget/answer-form', compact('post', 'thread', 'edit', 'populatedData', 'answer'));
    	return $cmaPageContent;
    }
    ...
    protected static function _processEditQuestion() {
    	...
    	if (isset($_GET['editQuestionId']) AND is_numeric($_GET['editQuestionId']) AND $thread = CMA_Thread::getInstance($_GET['editQuestionId'])
	    	...
	    	try {
	    		...
	    		// Update attachments
	    		$newIds = (!empty($_POST['cma_attachments']) ? $_POST['cma_attachments'] : array());
	    		$oldIds = $thread->getAttachmentsIds();
	    		$toDelete = array_diff($oldIds, $newIds);
	    		foreach ($toDelete as $attachmentId) {
	    			$thread->deleteAttachment($attachmentId);
	    		}
    ...
    protected static function _processEditAnswer($answerId) {
    	...
    	if ($thread AND $answer AND $answer->canEdit()) {
	    	...
	    	try {
	    		...
	    		$answer->updateContent(wpautop($_POST['content']), get_current_user_id());
	    		
	    		// Update attachments
	    		$newIds = (!empty($_POST['cma_attachments']) ? $_POST['cma_attachments'] : array());
	    		$oldIds = $answer->getAttachmentsIds();
	    		$toDelete = array_diff($oldIds, $newIds);
	    		foreach ($toDelete as $attachmentId) {
    ...
    protected static function _processAddThread()
    {
        ...
        $data = array(
        	...
        	'relatedPostId' => (isset($_POST['cma-related-post']) ? $_POST['cma-related-post'] : null),
        );
        ...
        if( isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest' )
        {
            ...
            header('Content-type: application/json');
            echo json_encode(apply_filters('cma_thread_created_ajax_json_response', array(
            	'success'   => (int) (!$error),
            	'thread_id' => $thread_id,
                'messages'   => $messages,
            ), (empty($thread) ? null : $thread)));
            exit;
        }
        else
        {
            ...
            else
            {
                ...
                if (!empty($_POST['cma-redirect'])) {
                	if ($_POST['cma-redirect'] == '_thread') {
                		if ($heldForModeration OR !$thread->isVisible()) {
                			wp_redirect(!empty($_POST['cma-referrer']) ? $_POST['cma-referrer'] : get_post_type_archive_link(CMA_Thread::POST_TYPE), 303);
...
//                 			}
    ...
    protected static function _processLoadSubcategories() {
    	...
    	if (!CMA_Settings::getOption(CMA_Settings::OPTION_ALLOW_POST_ONLY_SUBCATEGORIES)) {
    		...
    		$result[] = array('id' => '', 'name' => CMA_Labels::getLocalized('select_subcategory'), 'url' => '');
    	}
    	if (!empty($categoryId)) {
    		...
    		foreach ($_FILES['cma-file']['name'] as $i => $name) {
    			...
    			if ($_FILES['cma-file']['size'][$i] > $maxFileSize) {
	    			...
	    			if( move_uploaded_file($_FILES['cma-file']['tmp_name'][$i], $target) ) {
	    				...
	    				$url = wp_get_attachment_image_url($attach_id, CMA_Attachment::IMAGE_SIZE_MEDIUM);
    	...
    	if( self::$query->is_single() ) {
    		...
    		if( !empty($post) ) {
    			...
    			if (empty($_POST['nonce']) OR !wp_verify_nonce($_POST['nonce'], 'cma_report_spam')) {
    				...
    				$answer->markAsSpam(0);
    				$response['success'] = 1;
    				$response['message'] = CMA_Labels::__('unmarked_as_spam');
    			}
...
// 		var_dump($params);
	...
	public static function indexHeaderAfter() {
		...
		if (CMA_Settings::getOption(CMA_Settings::OPTION_THREAD_SHOW_TITLE)) {
			echo self::_loadView('answer/meta/index-header', compact('thread'));
		}
	...
	public static function addedTermRelationship($object_id, $tt_id) {
		...
		if (CMA_Category::TAXONOMY == $wpdb->get_var($wpdb->prepare("SELECT taxonomy FROM $wpdb->term_taxonomy WHERE term_taxonomy_id = %d", $tt_id))) {
			...
			$terms = wp_get_post_terms($object_id, CMA_Category::TAXONOMY);
			$toRemove = array_filter(array_map(function(WP_Term $term) use ($tt_id) {
				if ($term->term_taxonomy_id == $tt_id) return null;
				else return $term->term_id;
			}, $terms));
			
			if (!empty($toRemove)) {
				$sql = $wpdb->prepare("DELETE FROM $wpdb->term_relationships
						WHERE object_id = %d
						AND term_taxonomy_id IN (". implode(',', $toRemove) .")",
						$object_id);
// 				var_dump($sql);exit;
				$wpdb->query($sql);
			}
	...
	static function cma_edit_question_attachments(CMA_Thread $thread) {
		$attachments = $thread->getAttachments();
		echo self::_loadView('answer/meta/attachments-edit', compact('attachments'));
	}
	
	
	static function cma_edit_answer_attachments(CMA_Answer $answer) {
		$attachments = $answer->getAttachments();
		echo self::_loadView('answer/meta/attachments-edit', compact('attachments'));
	}
	...
	static function fixBuddyPressGlobalSearch($template) {
		if( static::$query->get('post_type') == CMA_Thread::POST_TYPE || static::$query->is_tax(CMA_Category::TAXONOMY) ) {
			remove_filter( 'template_include', 'buddyboss_global_search_result_page_dummy_post_load', 999 );
		}
		return $template;
	}
	
	
	static function supportTagsSearch($search, WP_Query $query) {
		global $wpdb;
		$option = CMA_Settings::getOption(CMA_Settings::OPTION_SEARCH_INCLUDE_TAGS);
		if ($query->is_main_query() AND $query->is_search() AND empty( $query->query_vars['suppress_filters'] )
				AND $option != CMA_Settings::SEARCH_INCLUDE_TAGS_DISABLE) {
					$pos = strpos($search, ' AND ');
					if ($pos !== false) {
						if (strlen($query->get('s')) == 0 AND !empty($_GET['search'])) {
							$query->set('s', $_GET['search']);
						}
// 						var_dump($search);exit;
						if (substr($query->get('s'), 0, 1) == '#') {
							$search = ' AND (0=1)';
							$query->set('s', str_replace('#', '', $query->get('s')));
						}
						
						if ($option == CMA_Settings::SEARCH_INCLUDE_TAGS_SINGLE) {
							$words = array($query->get('s'));
						} else {
							$words = array_filter(explode(' ', $query->get('s')));
						}
						
						$sql = PHP_EOL;
						foreach ($words as $i => $word) {
							if ($i > 0) $sql .= ' '. ($option == CMA_Settings::SEARCH_INCLUDE_TAGS_MULTIPLE_ONE ? 'OR' : 'AND') .' ';
							$sql .= $wpdb->prepare("($wpdb->posts.post_type = %s AND (
									cma_tag_$i.name = %s
									OR cma_tag_$i.name LIKE %s
									OR cma_tag_$i.name LIKE %s
									OR cma_tag_$i.name LIKE %s)) ",
									CMA_Thread::POST_TYPE, $word, "$word %", "% $word", "% $word %") . PHP_EOL;
	
						}
						 
						if (strlen(trim($sql)) > 0) {
							$search = " AND (\n( /* CMA search tags support */ ". $sql . ")\nOR\n ". substr($search, $pos + 5, strlen($search)) .' )';
							// 	    			var_dump($search);exit;
	
							// Add required joins
							$joinFunc = function($join, WP_Query $query) use ($words, &$joinFunc) {
								global $wpdb;
								foreach ($words as $i => $word) {
									$join .= " LEFT JOIN $wpdb->term_relationships cma_tag_tr_$i ON cma_tag_tr_$i.object_id = $wpdb->posts.ID
									LEFT JOIN $wpdb->term_taxonomy cma_tag_tt_$i ON cma_tag_tt_$i.term_taxonomy_id = cma_tag_tr_$i.term_taxonomy_id
									AND cma_tag_tt_$i.taxonomy = 'post_tag'
									LEFT JOIN $wpdb->terms cma_tag_$i ON cma_tag_$i.term_id = cma_tag_tt_$i.term_id";
								}
								remove_filter('posts_join', $joinFunc, 10);
								return $join;
							};
							add_filter('posts_join', $joinFunc, 10, 2);
							// Group by post ID
							add_filter('posts_groupby', array(__CLASS__, 'supportTagsSearchGroupBy'), 10, 2);
	...
	static function supportTagsSearchGroupBy($groupby, WP_Query $query) {
		global $wpdb;
		$groupby = "{$wpdb->posts}.ID";
		remove_filter('posts_groupby', array(__CLASS__, __FUNCTION__), 10);
		return $groupby;
	}
	
	
	
	static function cma_index_questions_top($displayOptions) {
		
		$categoryId = (empty($displayOptions['categoryId']) ? null : $displayOptions['categoryId']);
		
		// Question form on top
		if( !empty($displayOptions['form']) AND !empty($displayOptions['formontop']) ) {
			do_action('cma_index_question_form', $categoryId, 'top', $displayOptions);
		}
		
		// Follow bar
		$cssClassName = 'questions-top';
		echo static::getIndexPageFollowBar($categoryId, $cssClassName);
		
	}
	
	
	static function cma_questions_index_bottom($displayOptions) {
		
		$categoryId = (empty($displayOptions['categoryId']) ? null : $displayOptions['categoryId']);
		
		// Follow bar
		$cssClassName = 'questions-index-bottom';
		echo static::getIndexPageFollowBar($categoryId, $cssClassName);
		...
		if( !empty($displayOptions['form']) AND empty($displayOptions['formontop']) ) {
			...
			do_action('cma_index_question_form', $categoryId, 'bottom', $displayOptions);
		}
		
	}
	
	
	static function getIndexPageFollowBar($categoryId = null, $cssClassName = null) {
		$category = ($categoryId ? CMA_Category::getInstance($categoryId) : null);
		return static::_loadView('answer/meta/follow-bar', compact('category', 'cssClassName'));
	}
