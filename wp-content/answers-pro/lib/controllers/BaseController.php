...
//             self::initSessions();
    ...
    public static function initialize()
    {
    	...
    	add_action('wp_ajax_cma_debug_email', array(__CLASS__, 'ajax_cma_debug_email'));
    }
...
//     			self::$query->is_home = true;
    ...
    static function filterTitle($title) {
    	...
    	if (is_main_query() AND is_single() AND get_query_var('cma_prepared_single') AND $cmaTitle = get_query_var('cma_title')
    			AND ($title == str_replace('-', '&#8211;', $fakePost->post_title) OR $title == 'Questions')) {
    		$title = $cmaTitle;
    	}
    	return $title;
    }
    ...
    protected static function initFakePage() {
    	...
    	if (empty($record)) { // Post does not exists
    		...
    		if ($record->post_status != 'future' AND $record->post_status != 'private') {
    ...
    static function getFakePost() {
    	...
    	static $post = null;
    	if (isset($post) AND is_object($post) AND $post instanceof \WP_Post) {
    		return $post;
    	}
...
//     	});
    	add_action('add_meta_boxes', function($post_type, $post) {
    		if (!$post) return;
    		$postTypes = array_keys(CMA_Settings::getPostTypesOptions());
    		foreach ($postTypes as $postType) {
    			if ($postType == CMA_Thread::POST_TYPE) continue;
    			add_meta_box(
    					'cma-replace-comments',
    					__( 'CM Answers: Replace Comments' ),
    					function($post) {
    						$nonce = wp_create_nonce('cma_replace_comments_nonce');
    						$categories = CMA_Category::getCategoriesTree(null, 0, $onlyVisible = false);
    						$currentCategory = CMA_Category::getPostCommentsCategory($post->ID);
    						echo CMA_BaseController::_loadView('../backend/hooks/post_comment_status_meta_box', compact('categories', 'post', 'currentCategory', 'nonce'));
    					},
    					$postType,
    					'normal',
    					'default'
    	...
    	add_action('save_post', function($postId) { // Save CMA category ID for post/page
    		if ($nonce = filter_input(INPUT_POST, 'cma_replace_comments_nonce') AND wp_verify_nonce($nonce, 'cma_replace_comments_nonce')) {
                // post_category
                // cma_category
    			if (empty($_POST['cma_category'])) {
    				wp_set_post_terms($postId, array(), CMA_Category::TAXONOMY, true);
    			} elseif ($_POST['cma_category'] == 'new') {
    				if (!empty($_POST['cma_category_name'])) {
    					$category = (object)wp_insert_term($_POST['cma_category_name'], CMA_Category::TAXONOMY);
    				    clean_term_cache(array($category->term_id), CMA_Category::TAXONOMY, true);
    				}
    			} else {
...
//     	}, 10);
    ...
    public static function _userRequired()
    {
        ...
        {
            self::_addError('You have to be logged in to see this page. <a href="' . esc_attr(wp_login_url($_SERVER['REQUEST_URI'])) . '" rel="nofollow">Log in</a>');
            return false;
        }
        return true;
    }
...
//             add_submenu_page(apply_filters('CMA_admin_parent_menu', 'options-general.php'), CMA_Settings::__('User Guide'), CMA_Settings::__('User Guide'), 'manage_options', self::ADMIN_USER_GUIDE, array(get_class(), 'displayAboutPage'));
    ...
    public static function displayAboutPage()
    {
        ...
        if ($_GET['page'] == self::ADMIN_ABOUT) {
        	...
        	$iframeURL = 'https://www.cminds.com/wordpress-plugins-knowledge-base-and-documentation/?hscat=987-answers-cma';
        } else {
	...
	public static function get_header($name = null) {
		...
		if (!CMA_Settings::getOption(CMA_Settings::OPTION_SEO_META_REWRITE_ENABLED)
				OR isset($_GET['tcb_lightbox']) // fix for the Thrive Lightbox
			) {
// 			get_header($name);
			return;
		}
		...
		if ($wp_query->get('CMA-contributor-index')) {
			...
			if (isset($obj->term_id) AND $category = CMA_Category::getInstance($obj->term_id)) {
				...
				if (CMA_Settings::getOption(CMA_Settings::OPTION_ANSWERS_AS_HOMEPAGE)) {
					$canonical = site_url('/');
				} else {
...
// 		wp_enqueue_script('cma-test-ajax', CMA_RESOURCE_URL . 'js/test-ajax.js', array(), false, true);
        ...
        if ((!CMA_Settings::getOption(CMA_Settings::OPTION_DISABLE_CSS) AND !defined('CMA_CSS_PRINTED')) && $style = CMA_Settings::getOption(CMA_Settings::OPTION_LOOK_AND_FEEL_STYLE)) {
            wp_enqueue_style('cma-look-and-feel-style', CMA_URL . '/views/resources/css/'. $style .'.css', array('CMA-css'), CMA::version());
        }
	...
	static function ajax_cma_debug_email() {
		$nonce = filter_input(INPUT_POST, 'nonce');
		$email = filter_input(INPUT_POST, 'email');
		if (wp_verify_nonce($nonce, 'cma_debug_email')) {
			if (is_email($email)) {
				$subject = '['. get_bloginfo('blogname') .'] CM Answers testing email';
				$body = 'This is a test email sent ' . Date('Y-m-d H:i:s');
				if ($result = CMA_Email::send($email, $subject, $body)) {
					echo 'Email has been delegated to send successfully. Please check your email account to confirm if there is a new message.';
				} else die('Error when sending email.');
