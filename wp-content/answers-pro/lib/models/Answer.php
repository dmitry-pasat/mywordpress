...
class CMA_Answer {
	...
	public static function getAnswersByThread($threadId, $approved = true, $orderby = self::ORDER_BY_DATE, $sort = null, $limit = null) {
		...
		if( $orderby == CMA_Answer::ORDER_BY_VOTES ) {
			...
			$order = ' ORDER BY IFNULL(cmv.meta_value, 0) ';
		} else {
	...
	public function getLightContent() {
		return CMA_Thread::lightContent($this->getContent());
	}
	...
	public function saveContent($content) {
		...
		if ($content == $this->comment->comment_content) {
			...
			// WP removes formatting for subscribers so update it directly in the database:
			$rval = $wpdb->update($wpdb->comments, array('comment_content' => $content), array('comment_ID' => $this->getId()));
...
// 			$rval = $wpdb->update($wpdb->comments, (array)$this->comment, array('comment_ID' => $this->getId()));
	...
	function setCommentEmail($email) {
	...
	function deleteAttachment($attachmentId) {
		if ($post = get_post($attachmentId) AND $attachment = new CMA_AnswerAttachment($post)) {
			$attachment->delete();
			return delete_comment_meta($this->getId(), CMA_Answer::META_ATTACHMENT, $attachmentId);
		}
	}
	...
	function getAttachmentsIds() {
		$res = get_comment_meta($this->getId(), CMA_Answer::META_ATTACHMENT, false);
	...
	function canPostComments($userId = null) {
		if ($thread = $this->getThread()) {
			$result = $thread->canPostComments($userId);
		} else {
			$result = false;
		}
