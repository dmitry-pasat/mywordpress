...
class CMA_Category {
	...
	public static function getAll($onlyVisible = true) {
		...
		foreach ($terms as $term) {
			$category = new static($term);
			if ($category->isVisible()) {
				$out[$term->term_id] = $category;
			}
		}
		return $out;
	}
...
//     			var_dump($result);exit;
    ...
    public static function getExpertsTermTaxonomyIds($userId = null) {
    	...
    	if (empty($results[$userId])) {
    		...
    		// Get terms ids for this expert
    		$options = $wpdb->get_col($wpdb->prepare("SELECT option_name FROM $wpdb->options WHERE option_name LIKE %s",
    			static::OPTION_EXPERTS_PREFIX . '\_%\_' . $userId));
    		
    		$termsIds = array_unique(array_filter(array_map(function($name) {
    			$name = explode('_', $name);
    			$userId = array_pop($name);
    			$categoryId = array_pop($name);
    			return $categoryId;
    		}, $options)));
    		
    		// Get term_taxonomy_id
    		if (!empty($termsIds)) {
    			$ttIds = $wpdb->get_col($wpdb->prepare("SELECT term_taxonomy_id FROM $wpdb->term_taxonomy
    				WHERE taxonomy = %s AND term_id IN (". implode(',', $termsIds) .")", static::TAXONOMY));
    			$results[$userId] = $ttIds;
    		}
