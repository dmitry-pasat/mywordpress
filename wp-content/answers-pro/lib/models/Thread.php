...
class CMA_Thread extends CMA_PostType {
    ...
    protected static $answersNumberCache = array();
...
//         $content = self::contentFilter($this->post->post_content, $this->getAuthorId());
        ...
        {
            $content = mb_substr($content, 0, $charscount) . "...";
        }
        return $content;
    }
    
    
    public function getLightContent($charsLimit = 0) {
    	return self::lightContent($this->getContent($charsLimit));
    }
    ...
    function getAttachmentsIds() {
    	$res = $this->getPostMeta(CMA_Thread::$_meta['attachment'], false);
    ...
    {
    	return static::getUser($this->getAuthorId(), $this);
    }
    ...
    public function getAuthorLink($simple = false) {
    	if ($user = $this->getAuthor()) {
    		if (CMA_Settings::getOption(CMA_Settings::OPTION_ANONYMOUS_USER_ENABLE) AND $user->ID != CMA::getAnonymousPostingUserId()) {
     ...
     * @param unknown $contextObject
     * @return CMA_User
     */
    public static function getUser($userId, $contextObject = null) {
    	if ($userId AND $user = apply_filters('cma_filter_author', CMA_User::get_user_by('id', $userId), $contextObject)) {
    ...
    public function getTitle($withPrefixes= true)
    {
        $title = '';
        if( $this->isResolved() AND $withPrefixes AND CMA_Settings::getOption(CMA_Settings::OPTION_ADD_RESOLVED_QUESTION_TITLE_PREFIX) ) {
//         	var_dump(CMA_Settings::getOption(CMA_Settings::OPTION_ADD_RESOLVED_QUESTION_TITLE_PREFIX));exit;
        	$title .= '[' . CMA_Labels::getLocalized('RESOLVED') . '] ';
        }
        $title .= parent::getTitle();
        return apply_filters('cma_get_question_title', $title, $this, $withPrefixes);
    }
...
//         return date_i18n($format, strtotime($dateString));
    ...
    public function getNumberOfAnswers() {
    	...
    	$id = $this->getId();
    	if (isset(static::$answersNumberCache[$id])) {
    		return static::$answersNumberCache[$id];
    	}
    	
    	$val = intval($wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->comments
    	...
    	static::$answersNumberCache[$id] = $val;
    ...
    static function prepareAnswersNumberCache(array $questions) {
    	
    	global $wpdb;
    	
    	$ids = array_filter(array_map(function($question) {
    		if (is_numeric($question)) return $question;
    		else if ($question instanceof \WP_Post) return $question->ID;
    		else if ($question instanceof CMA_Thread) return $question->getId();
    		else return 0;
    	}, $questions));
    	
    	if (empty($ids)) return;
    	
    	$results = $wpdb->get_results($wpdb->prepare("SELECT comment_post_ID, COUNT(*) AS num FROM $wpdb->comments
    			WHERE comment_post_ID IN (". implode(',', $ids) .")
    				AND comment_type = %s
    				AND comment_approved = 1
					GROUP BY comment_post_ID", CMA_Answer::COMMENT_TYPE), ARRAY_A);
    	
    	foreach ($results as $row) {
    		static::$answersNumberCache[$row['comment_post_ID']] = $row['num'];
    	}
    ...
    public static function newThread($data = array())
    {
        ...
        self::validateCategories( $data['category'], $editId, $errors);
        ...
        else
        {
            ...
            if (!empty($data['relatedPostId'])) {
            	$instance->addRelatedPostTags($data['relatedPostId']);
            }
            ...
            if( $status == 'draft' ) {
            	CMA_DebugLog::log('post_status=draft notify moderator');
                $instance->notifyModerator();
            } else {
            	CMA_DebugLog::log('post_status=publish notify about new question');
    ...
    function addRelatedPostTags($postId) {
    	$tags = wp_get_post_tags($postId, array('fields' => 'names'));
    	wp_set_post_tags($this->getId(), $tags, $append = true);
    	return $this;
    }
    ...
    protected static function validateCategories($category_id, $editId, &$errors) {
    	
    	// Category is required
    	if (CMA_Settings::getOption(CMA_Settings::OPTION_QUESTION_REQUIRE_CATEGORY)) {
	    	if (empty($category_id)) {
	    		$errors[] = CMA_Labels::getLocalized('question_validation_category_required_error');
	    	}
    	}
    	
    	// Only subcategories
    	if (CMA_Settings::getOption(CMA_Settings::OPTION_ALLOW_POST_ONLY_SUBCATEGORIES)) {
    		$category = CMA_Category::getInstance( (int)$category_id );
    		if (empty($category_id) OR !$category OR !$category->getParentId() ) {
    			$errors[] = CMA_Labels::getLocalized('question_validation_must_be_subcategory_error');
    		}
    	}
    	
    	// This is not working at all:
//     	if ( empty($category_id) || (int)$category_id <= 0 ) {
//     	    $errors[] = CMA::__('Category cannot be empty');
// 	    } else {
//     		$allowPostOnlySubcat = CMA_Settings::getOption(CMA_Settings::OPTION_ALLOW_POST_ONLY_SUBCATEGORIES);
//     		$catFilterMode = CMA_Settings::getOption(CMA_Settings::OPTION_CATEGORY_FILTER_MODE);
//     		if( $allowPostOnlySubcat && $catFilterMode == CMA_Settings::CATEGORY_FILTER_MODE_TWO_LEVEL ) {
//     		    $category = CMA_Category::getInstance( (int)$category_id );
//     		    if( $category->getParentInstance() ) {
// 					$errors[] = CMA::__('Please select subcategory');
// 		        }
// 		    }
...
// 	    }
    ...
    public function isVisible($userId = null) {
    	...
    	if ($result AND CMA_Settings::getOption(CMA_Settings::OPTION_RESTRICT_UNANSWERED_QUESTIONS_TO_EXPERTS)) {
    		$result = ($this->getAnswersCount(false /* must be false, else the recursion occurs */) > 0
    				OR $this->isExpert($userId)
    				OR $this->getAuthorId() == $userId);
    	}
//     	var_dump($result);
    	return apply_filters('cma_thread_is_visible', $result, $this->getId(), $this);
...
";
		...
		CMA_DebugLog::log($emailTitle, $receivers);
...
// 		var_dump($receivers);exit;
    ...
    public function addAnswer($content, $author_id, $follow = false, $resolved = false, $private = false)
    {
        ...
        $content = apply_filters('comment_text', str_replace(';)', ':)', $content));
//         var_dump($content);exit;
    ...
    function delete() {
    	$id = $this->getId();
    	$res = wp_delete_post($id, $force = !CMA_Settings::getOption(CMA_Settings::OPTION_QUESTION_DELETE_TRASH)) !== false;
    	if ($res) {
    		do_action('cma_thread_deleted', $id);
    	}
    	return $res;
    }
    ...
    public function canPostAnswers($userId = null)
    {
        ...
        switch($postAnswersSetting)
        {
            ...
            case CMA_Settings::ACCESS_ROLE:
                {
                    ...
                    $hasRightRole = array_intersect($accessRoles, $userRoles);
                    $result = user_can($user, 'manage_options') || !empty($hasRightRole);
                    break;
                }
            case CMA_Settings::ACCESS_EXPERT:
            	if ($this->hasExperts()) {
            		$result = $this->isExpert($userId);
            	} else {
            		$result = is_user_logged_in();
            	}
            	break;
            ...
            default:
            	$result = (apply_filters('cma_can_post_answers', $postAnswersSetting) OR is_user_logged_in());
                break;
        }
        ...
        if (!CMA_Settings::getOption(CMA_Settings::OPTION_ANSWER_AFTER_RESOLVED) AND $this->isResolved()) {
        	$result = false;
        }
        
        return apply_filters('cma_can_post_answers_final', $result, $userId, $this);
        
    }
    
    
    function canPostComments($userId = null) {
    	$result = CMA_Comment::canCreate($userId);
    	if (!CMA_Settings::getOption(CMA_Settings::OPTION_ALLOW_COMMENTS_IN_RESOLVED_THREAD) AND $this->isResolved()) {
    		$result = false;
    	}
    	return apply_filters('cma_can_post_comments_final', $result, $userId, $this);
    }
    ...
    public static function getCategorizedThreadIdsSubquery() {
    	...
    	return $wpdb->prepare("SELECT tr.object_id
			...
			JOIN $wpdb->posts p ON p.ID = tr.object_id AND p.post_type = %s
    		WHERE tt.taxonomy = %s
    		", CMA_Thread::POST_TYPE, CMA_Category::TAXONOMY);
    	
    }
    
    
    static function getCategoriesSubquery($postTableAlias, $categoryTaxonomy, $selectField = 'term_taxonomy_id') {
    	global $wpdb;
    	return $wpdb->prepare("
	        SELECT  cma_twc_rel.{$selectField}
	        FROM    {$wpdb->term_relationships} cma_twc_rel
	        JOIN    {$wpdb->term_taxonomy} cma_twc_tax
	        	ON      cma_twc_tax.term_taxonomy_id = cma_twc_rel.term_taxonomy_id 
	            	    AND cma_twc_tax.taxonomy = %s 
	        JOIN    {$wpdb->terms} cma_twc_term
	        	ON      cma_twc_term.term_id = cma_twc_tax.term_id
	        WHERE   {$postTableAlias}.ID = cma_twc_rel.object_id
        ", $categoryTaxonomy, CMA_Thread::POST_TYPE);
    }
    ...
    public static function getCategoryAccessFilterSubquery($userId = null) {
    	...
    	if (user_can($userId, 'manage_options')) { // Admin can view all categories
	    	...
	    	$sql = "SELECT tr.object_id
	    		...
	    		WHERE " . $wpdb->prepare("p.post_type =  %s ", CMA_Thread::POST_TYPE);
	    	if (!CMA_Category::isAnyCategoryResticted()) {
	    		// all categories are visible:
// 	    		$sql .= " AND 1=1 ";
	    	}
	    	else if ($ids = CMA_Category::getVisibleTermTaxonomyIds($userId)) {
	    	...
	    	if (CMA_Settings::getOption(CMA_Settings::OPTION_RESTRICT_UNANSWERED_QUESTIONS_TO_EXPERTS)) {
	    		$sql .= " AND (p.comment_count > 0"; // question is not unanswered
	    		...
	    		$sql .= ")";
// 	    		echo '<br><br><br><br><br><br><br><br>';var_dump($sql);
	    	}
	    	return $sql;
    	}
    ...
    public function getPermalink(array $query = array(), $backlink = false, $append = '') {
    	...
    	if ($backlink === true AND CMA_Settings::getOption(CMA_Settings::OPTION_BACKLINK_PARAM_ENABLED)) {
    		if ($override_backlink = filter_input(INPUT_GET, 'override_backlink')) {
    			$query['backlink'] = $override_backlink;
    		}
    		else if ($override_backlink = filter_input(INPUT_POST, 'override_backlink')) {
    			$query['backlink'] = $override_backlink;
    		} else {
...
//     }
    ...
    public function deleteAttachment($attachmentId) {
    	if ($post = get_post($attachmentId) AND $attachment = new CMA_QuestionAttachment($post)) {
    		$attachment->delete();
    		return delete_post_meta($this->getId(), CMA_Thread::$_meta['attachment'], $attachmentId);
    	}
    }
    ...
    public static function canSetStage($userId = null) {
    	if (is_null($userId)) $userId = get_current_user_id();
//     	var_dump($userId);
    	$res = user_can($userId, 'publish_posts');
//     	var_dump($res);
    	return $res;
    }
    
    
    function getStage($includeDefault = true) {
    	$stage = $this->getPostMeta(static::$_meta['stage']);
    	if ($includeDefault AND strlen($stage) == 0) {
    		$stage = static::getDefaultStage();
    	}
    	return $stage;
    }
    ...
    function setStage($stage) {
    	return $this->setPostMeta(static::$_meta['stage'], $stage);
    }
    
    
    static function getPossibleStages() {
    	return array_map('trim', explode("\n", CMA_Settings::getOption(CMA_Settings::OPTION_QUESTION_STAGES_LIST)));
    }
    
    
    static function getDefaultStage() {
    	$stages = static::getPossibleStages();
    	return reset($stages);
    }
    
    
    static function getLastStage() {
    	$stages = static::getPossibleStages();
    	return end($stages);
    }
    ...
    static function reverse_wpautop($s)
    {
