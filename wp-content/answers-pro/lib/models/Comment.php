...
class CMA_Comment {
	...
	public static function create($content, $userId, $threadId, $answerId = null) {
		...
		if ($limit = CMA_Settings::getOption(CMA_Settings::OPTION_JS_LIMIT_QUESTION_COMMENT)) {
			if (strlen($content) > $limit) {
				throw new Exception(sprintf(CMA_Labels::getLocalized('error_comment_content_too_long'), $limit));
			}
		}
	...
	public static function canCreate($userId = null) {
		...
		switch ($access) {
			...
			case CMA_Settings::ACCESS_ROLE:
				...
				break;
			default:
				$result = (apply_filters('cma_can_post_comments', $access) OR is_user_logged_in());
		}
		return $result;
	}
