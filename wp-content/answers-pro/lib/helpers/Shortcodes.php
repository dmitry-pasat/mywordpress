...
class CMA_Shortcodes
{
    ...
    public static function init()
    {
        ...
        add_shortcode('cma-favorite-questions', array(__CLASS__, 'shortcode_favorite_questions'));
...
//         add_shortcode('cma-get-param', function($args) { if (isset($args['name'])) { return strip_tags(filter_input(INPUT_GET, $args['name'])); } });
        add_shortcode('cma-get-param-author', function($args) { return strip_tags(filter_input(INPUT_GET, 'author')); });
...
//         add_action('wp_enqueue_scripts', array(__CLASS__,'addStyles'));
	...
	public static function shortcode_answers($atts)
    {
    	...
    	$atts['author'] = CMA_Shortcodes::doShortcodeInParam($atts['author']);
...
//         $answersCount = CMA_Answer::countForUser($atts['author'], $approved = true, $onlyVisible = true);
    ...
    public static function shortcode_my_questions($atts, $widget = false)
    {
        ...
        if (!isset($atts['form'])) $atts['form'] = 0;
        if (!isset($atts['pagination'])) $atts['pagination'] = 1;
        if (!isset($atts['user_questions'])) $atts['user_questions'] = 1;
        if (!isset($atts['statusinfo'])) $atts['statusinfo'] = 1;
...
//         ), $atts);
    ...
    public static function shortcode_favorite_questions($atts, $widget = false) {
    	if (!isset($atts['form'])) $atts['form'] = 0;
    	$atts['favorite'] = 1;
    	return self::general_shortcode($atts, $widget);
    }
...
// 	    	$atts['displaycategories'] = 1;
    ...
    public static function general_shortcode($atts, $widget = true)
    {
        ...
        $atts = CMA_Thread::sanitize_array($atts, array(
        	...
        	'scrollaftersearch' => array('bool', 1),
        	'relatedpostid' => array('int', 0),
        	'favorite' => array('int', 0),
        ));
...
//             'order' => $atts['order'],
        ...
        if (!is_null($atts['resolved'])) {
       		$metaQuery = (!empty($questionsArgs['meta_query']) ? $questionsArgs['meta_query'] : array());
        	$metaQuery[] = array(
        		'key' => CMA_Thread::$_meta['resolved'],
        		'value' => intval($atts['resolved']),
        	);
        	$questionsArgs['meta_query'] = $metaQuery;
        }
        if (!empty($atts['favorite'])) {
        	$metaQuery = (!empty($questionsArgs['meta_query']) ? $questionsArgs['meta_query'] : array());
        	$metaQuery[] = array(
        		'key' => CMA_Thread::$_meta['usersFavorite'],
        		'value' => intval(get_current_user_id()),
        	);
        	$questionsArgs['meta_query'] = $metaQuery;
        }
...
//         var_dump($q->request);exit;
        ...
        $displayOptions = array(
        	...
        	'relatedpostid' => $atts['relatedpostid'],
        );
        $checkPermissions = true;
        $widget = true;
        ...
        if ($atts['ajax']) $result = '<div class="cma-widget-ajax" data-widget-cache-id="'. $widgetCacheId
        	.'" data-scrollaftersearch="'. intval($atts['scrollaftersearch']) .'">'. $result .'</div>';
        return $result;
    }
