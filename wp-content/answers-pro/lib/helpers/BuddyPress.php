...
// 		include CMA_PATH . '/views/frontend/buddypress/user-questions-loop.phtml';
	...
	static function thread_posted_activity( CMA_Thread $instance ) {
		...
		$action = sprintf(
			CMA_Labels::getLocalized('bp_activity_user_asked_question'), //CMA::__( '%s asked a new question: %s' ),
		...
		if (function_exists('bp_activity_add')) bp_activity_add( apply_filters('cma_thread_bp_activity_add', array(
			'action' => $action,
			'content' => $content,
		...
		if ($category = $instance->getCategory()) {
			...
			if ($groups) {
				foreach ($groups as $groupId) {
					if (function_exists('groups_record_activity')) groups_record_activity( apply_filters('cma_thread_bp_groups_record_activity', array(
						'action' => $action,
						'content' => $content,
	...
	static function answer_posted_activity( CMA_Thread $instance, CMA_Answer $answer ) {
		...
		$action = sprintf(
			CMA_Labels::getLocalized('bp_activity_user_answered_to_question'), // CMA::__( '%s answered to the question "%s"' ),
			bp_core_get_userlink( $user_id ),
			sprintf('<a href="%s">%s</a>',
				esc_attr($permalink),
				esc_html($instance->getTitle())
		...
		if (function_exists('bp_activity_add')) bp_activity_add( apply_filters('cma_answer_bp_activity_add', array(
			'action' => $action,
			'content' => $content,
			'component' => self::COMPONENT,
			'type' => 'answer_posted',
		...
		if ($category = $instance->getCategory()) {
			...
			if ($groups) {
				foreach ($groups as $groupId) {
					if (function_exists('groups_record_activity')) groups_record_activity( apply_filters('cma_answer_bp_groups_record_activity', array(
						'action' => $action,
						'content' => $content,
						'type' => 'new_groupblog_post',
						'primary_link' => $permalink,
	...
	public static function formatNotification( $action, $item_id, $secondary_item_id, $total_items, $format = 'string' ) {
		...
		switch ($action) {
			...
			case self::ACTION_NOTIFICATION_ANSWER:
				if ($answer = CMA_Answer::getById($item_id) AND $thread = $answer->getThread()) {
					$text = sprintf(CMA_Labels::getLocalized('bp_notification_answer_in_thread'), sprintf('<a href="%s">%s</a>',
						esc_attr($thread->getPermalink()),
						esc_html($thread->getTitle())
					)) .': '. sprintf('<a href="%s">%s</a>',
		...
		$text = apply_filters('cma_bp_notification_text', $text, compact('action', 'item_id', 'secondary_item_id', 'total_items', 'format'));
		$link = apply_filters('cma_bp_notification_link', $link, compact('action', 'item_id', 'secondary_item_id', 'total_items', 'format'));
			...
			if (!empty($_GET['post_id']) AND $thread = CMA_Thread::getInstance($_GET['post_id'])) {
				...
				foreach ($usersIds as $userId) {
					if ($thread->isVisible($userId)) {
						...
						bp_notifications_add_notification(apply_filters('cma_bp_notifications_add_notification', $notification));
					}
				}
