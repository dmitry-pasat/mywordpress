	...
	public static function old_update() {
        ...
        if( empty($oldVersion) OR version_compare($oldVersion, $currentVersion, '<') )
        {
            ...
            if (version_compare($oldVersion, '2.1.7', '<')) {
            	$postsIds = $wpdb->get_col($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_type = %s", CMA_Thread::POST_TYPE));
// 	            $posts = get_posts(array('post_type' => CMA_Thread::POST_TYPE));
	            if (count($postsIds) < 1000) foreach ($postsIds as $postId) {
	            	update_post_meta($postId, CMA_Thread::$_meta['votes_question'], 0);
	...
	public static function update_2_5_6() {
		...
		$records = $wpdb->get_results($wpdb->prepare("SELECT p.ID, pm.meta_value
			FROM $wpdb->posts p
			LEFT JOIN $wpdb->postmeta pm ON pm.post_id = p.ID AND pm.meta_key = %s
			WHERE post_type = %s AND pm.meta_value IS NULL",
			CMA_Thread::$_meta['views'], CMA_Thread::POST_TYPE), ARRAY_A);
// 		var_dump($records);exit;
		foreach ($records as $row) {
// 			if (!get_post_meta($id, CMA_Thread::$_meta['views'], $single = true)) {
// 				update_post_meta($id, CMA_Thread::$_meta['views'], 0);
// 			}
			add_post_meta($row['ID'], CMA_Thread::$_meta['views'], 0);
		}
